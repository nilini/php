...
add_to_hash:
	...
	// 计算散列值(散列值范围在[-1,-nTableSize])
	nIndex = h | ht->nTableMask;
	// 把旧的值（散列值）保存到新插入的元素中
	// #define Z_NEXT(zval)	(zval).u2.next
	// 冲突时，新bucket的zval的u2.next保存旧bucket的位置
	Z_NEXT(p->val) = HT_HASH(ht, nIndex);
	// 再把新元素数组存储位置更新到hash映射表
	// 散列值（nIndex）->Bucket的存储位置（idx）
	/*
...
static zend_always_inline zval *_zend_hash_index_add_or_update_i(HashTable *ht, zend_ulong h, zval *pData, uint32_t flag ZEND_FILE_LINE_DC)
{
	...
	/* 检查数组是否已经分配空间，初始化时候并没有分配空间，只有在第一次插入时才会根据nTableSize的大小分配。
	** 分配完之后会把 HashTable->u.flags打上HASH_FLAG_INITIALIZED掩码，这样下次就不会再重复操作 
	*/
	if (UNEXPECTED(!(ht->u.flags & HASH_FLAG_INITIALIZED))) {
		//如果数组还没有分配arData内存，则最终调用zend_hash_real_ex()完成内存的分配
		CHECK_INIT(ht, h < ht->nTableSize);
		if (h < ht->nTableSize) {
			p = ht->arData + h;
			// packed hashtables
			// 对于连续的、整数索引的数组（真正的数组）而言，hash的东西没有多大意义
			// PHP 7中引入了 "packed hashtables" 这个概念
			// packed hashtables中，arHash(映射表,arData反向)数组为NULL，查找只会直接在arData中进行，
			// 例如要查找key为5的元素，那么这个元素将是arData[5]，或者不存在，没必要处理冲突
			goto add_to_packed;
		}
		goto add_to_hash;
	} else if (ht->u.flags & HASH_FLAG_PACKED) {
...
add_to_packed:
		...
		// flag的值有这些：HASH_ADD、HASH_UPDATE、HASH_ADD_NEW、HASH_ADD_NEXT
		...
		ht->nNumOfElements++;
		/* HT_INVALID_IDX作为一个特殊的标记，在转换表（hash表、映射表）表示：
		* 对应的数据节点没有有效的数据，直接跳过。
		* 哈希表之所以能极大的减少那些创建时就是空值的数组的开销，得益于他的两步的初始化过程：
		* 当新的哈希表被创建时，我们只创建两个转换表节点，并且都赋予HT_INVALID_IDX标记
		* static const uint32_t uninitialized_bucket[-HT_MIN_MASK] = {HT_INVALID_IDX, HT_INVALID_IDX};
		*/ 
		if (ht->nInternalPointer == HT_INVALID_IDX) {
			// nInternalPointer 用于hash遍历
			ht->nInternalPointer = h;
		}
		// ???
		zend_hash_iterators_update(ht, HT_INVALID_IDX, h);
		// nNextFreeElement自动确定数组索引，下一个索引。
		if ((zend_long)h >= (zend_long)ht->nNextFreeElement) {
			ht->nNextFreeElement = h < ZEND_LONG_MAX ? h + 1 : ZEND_LONG_MAX;
		}
		// 哈希值或者数组索引 (此处是数组索引)
		p->h = h;
		// 数组元素的键值 (键值是索引类型，此处为NULL)
		p->key = NULL;
		//复制值（zval val）
		ZVAL_COPY_VALUE(&p->val, pData);
