	...
	union {
		struct {
			ZEND_ENDIAN_LOHI_4(
				...
				zend_uchar    type_flags,   /* 可标识是否支持引用计数（zval.u1.type_flags&IS_TYPE_REFCOUNTED）。
											   支持写时复制的标识IS_TYPE_COPYABLE。
											   垃圾回收器会把标识为IS_TYPE_COLLECTABLE的变量（可能垃圾）收集起来
											   此外还有几个其他的标识含义 */
...
typedef struct _zend_refcounted_h {
	...
	union {
		struct {
			ZEND_ENDIAN_LOHI_3(
				zend_uchar    type,
				zend_uchar    flags,    /* used for strings & objects 如果是内部字符串(内部字符串不会用到引用计数)，此值为IS_STR_INTERNED*/ 
