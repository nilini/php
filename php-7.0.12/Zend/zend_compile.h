...
struct _zend_op_array {
	...
	int last_var; // php代码中定义的变量数：op_type为IS_CV的变量。 编译前此值为0，然后发现一个新变量这个值就加1
	uint32_t T;   // 临时变量数：op_type为IS_TMP_VAR、IS_VAR的变量
	zend_string **vars; // 这个数组在ast编译期间配合last_var来确定各个变量的编号，非常重要的一步操作
	...
	zval *literals; // 字面量（常量）数组，这些都是在php代码中定义的一些值

	int  cache_size; // 运行时缓存数组大小
	void **run_time_cache; // 运行时缓存，主要用于缓存一些操作数以便于快速获取数据。 
