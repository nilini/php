...
PHP_FUNCTION(array_fill)
{
	...
	if (EXPECTED(num > 0)) { // 填充的个数大于0
		if (sizeof(num) > 4 && UNEXPECTED(EXPECTED(num > 0x7fffffff))) { // 0x7fffffff ==‭2147483647‬ = ‭01111111111111111111111111111111‬
...
PHP_FUNCTION(array_fill_keys)
{
	...
	ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(keys), entry) {
		ZVAL_DEREF(entry);
		Z_TRY_ADDREF_P(val);
		/* 以数组key值为键，val为值。插入返回的数组return_value */
...
static inline zval *array_column_fetch_prop(zval *data, zval *name, zval *rv)
{
	...
	} else if (Z_TYPE_P(data) == IS_ARRAY) { // data为数组
		/* 按键值（name）获取数组data的值 */
...
PHP_FUNCTION(array_filter)
{
	...
	if (ZEND_NUM_ARGS() > 1) {
		have_callback = 1;   // 参数不只一个，则传入了回调函数
		fci.no_separation = 0;
		fci.retval = &retval;
		if (use_type == ARRAY_FILTER_USE_BOTH) { //回调函数同时接受键名和键值
			fci.param_count = 2;
			key = &args[1];
		} else {
			fci.param_count = 1;
...
PHP_FUNCTION(array_combine)
{
	...
	ZEND_HASH_FOREACH_VAL(keys, entry_keys) {
		/* 通过pos_value从前向后取第一个数组和第二个数组的元素值，直到数组尾 
		*  每次取出元素后组成键值对，插入返回数组。
		*/
		while (1) {
